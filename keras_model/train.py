...
def train_model(model, args, X_train, X_valid, y_train, y_valid):
    ...
    """
    # for SqueezeLSTMNet 
    model.fit_generator(batch_seq_generator(args.data_dir, X_train, y_train, args.batch_size, True),
                        args.samples_per_epoch/args.batch_size,
                        args.nb_epoch,
                        max_queue_size=1,
                        validation_data=batch_seq_generator(args.data_dir, X_valid, y_valid, args.batch_size, False),
                        validation_steps=len(X_valid),
                        callbacks=[checkpoint],
                        verbose=1)
    """
...
def main():
    ...
    data = load_data(args)
    #build model    
    drive_net = build_model(args)
    #load pre-trained weight
    if args.model_dir != '':
        drive_net.model_init(args.model_dir)
    else:
        drive_net.model_init()
    #train model on data, it saves as model.h5 
    train_model(drive_net.net, args, *data)
